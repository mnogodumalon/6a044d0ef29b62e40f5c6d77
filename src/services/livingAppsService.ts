...
import type { Test1223XxeErfassung, CreateTest1223XxeErfassung } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getTest1223XxeErfassung(): Promise<Test1223XxeErfassung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TEST_1223_XXE_ERFASSUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Test1223XxeErfassung[];
    return enrichLookupFields(records, 'test_1223_xxe_erfassung');
  }
  static async getTest1223XxeErfassungEntry(id: string): Promise<Test1223XxeErfassung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.TEST_1223_XXE_ERFASSUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Test1223XxeErfassung;
    return enrichLookupFields([record], 'test_1223_xxe_erfassung')[0];
  }
  static async createTest1223XxeErfassungEntry(fields: CreateTest1223XxeErfassung) {
    return callApi('POST', `/apps/${APP_IDS.TEST_1223_XXE_ERFASSUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'test_1223_xxe_erfassung') });
  }
  static async updateTest1223XxeErfassungEntry(id: string, fields: Partial<CreateTest1223XxeErfassung>) {
    return callApi('PATCH', `/apps/${APP_IDS.TEST_1223_XXE_ERFASSUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'test_1223_xxe_erfassung') });
  }
  static async deleteTest1223XxeErfassungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TEST_1223_XXE_ERFASSUNG}/records/${id}`);
  }
