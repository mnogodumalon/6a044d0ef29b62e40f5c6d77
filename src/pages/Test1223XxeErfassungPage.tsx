...
export default function Test1223XxeErfassungPage() {
  const [records, setRecords] = useState<Test1223XxeErfassung[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Test1223XxeErfassung | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Test1223XxeErfassung | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Test1223XxeErfassung | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getTest1223XxeErfassung());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Test1223XxeErfassung['fields']) {
    await LivingAppsService.createTest1223XxeErfassungEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Test1223XxeErfassung['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateTest1223XxeErfassungEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Test 1223 xxe Erfassung"
      subtitle={`${records.length} Test 1223 xxe Erfassung im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Test 1223 xxe Erfassung suchen..."
          value={search}
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Test 1223 xxe Erfassung. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <ConfirmDialog
        ...
        title="Test 1223 xxe Erfassung löschen"
