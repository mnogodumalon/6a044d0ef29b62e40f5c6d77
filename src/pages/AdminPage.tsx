...
import { Test1223XxeErfassungViewDialog } from '@/components/dialogs/Test1223XxeErfassungViewDialog';
...
const TEST1223XXEERFASSUNG_FIELDS = [
...
const ENTITY_TABS = [
  { key: 'test_1223_xxe_erfassung', label: 'Test 1223 xxe Erfassung', pascal: 'Test1223XxeErfassung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('test_1223_xxe_erfassung');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'test_1223_xxe_erfassung': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'test_1223_xxe_erfassung': return (data as any).test1223XxeErfassung as Test1223XxeErfassung[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'test_1223_xxe_erfassung': return TEST1223XXEERFASSUNG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'test_1223_xxe_erfassung': return {
        create: (fields: any) => LivingAppsService.createTest1223XxeErfassungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateTest1223XxeErfassungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteTest1223XxeErfassungEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'test_1223_xxe_erfassung' || dialogState?.entity === 'test_1223_xxe_erfassung') && (
        <Test1223XxeErfassungDialog
          open={createEntity === 'test_1223_xxe_erfassung' || dialogState?.entity === 'test_1223_xxe_erfassung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'test_1223_xxe_erfassung' ? handleUpdate : (fields: any) => handleCreate('test_1223_xxe_erfassung', fields)}
          defaultValues={dialogState?.entity === 'test_1223_xxe_erfassung' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Test1223XxeErfassung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Test1223XxeErfassung']}
        />
      )}
      {viewState?.entity === 'test_1223_xxe_erfassung' && (
        <Test1223XxeErfassungViewDialog
          open={viewState?.entity === 'test_1223_xxe_erfassung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'test_1223_xxe_erfassung', record: r }); }}
        />
      )}
